...
def record_directory_structure(folder_path):
...
def check_directory_structure(folder_path, record_file):
...
def copy_local_path_from_hdfs(
    ...
    """copy_local_path_from_hdfs is deprecated. Use copy_to_local instead."""
    ...
    if _is_non_local(src):
        ...
        with FileLock(lock_file=lock_file):
            ...
            if not os.path.exists(local_path):
                ...
                copy(src, local_path)
                if os.path.isdir(local_path):
                    record_directory_structure(local_path)
                ...
                if not check_directory_structure(local_path, record_file):
                    if verbose:
